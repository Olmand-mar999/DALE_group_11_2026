...
cut_off <- (nrow(collected_data)-4)
collected_data <- collected_data[1:cut_off,]
...
model <- lmm.forbrug.ftillid.ftillid
...
y_pred
y <- collected_data$Privat_forbrug
# 2. Calculate Sum of Squared Residuals (SSres) and Total Sum of Squares (SStot)
...
#outliers i y
box <- boxplot(collected_data$Privat_forbrug)
box$out
